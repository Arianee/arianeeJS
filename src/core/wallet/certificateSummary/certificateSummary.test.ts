  ...
  describe('It should build a certificate', () => {
    it('should be returning only set properties', () => {
      const arianeeCertificate = new CertificateSummaryBuilder()
        .setContent({ isAuthentic: true, data: { $schema: '' } } as any)
        .build();

      expect(arianeeCertificate.content).toBeDefined();

      expect(Object.prototype.hasOwnProperty.call(arianeeCertificate, 'events')).toBeFalsy();
      expect(Object.prototype.hasOwnProperty.call(arianeeCertificate, 'advanced')).toBeFalsy();
      expect(Object.prototype.hasOwnProperty.call(arianeeCertificate, 'issuer')).toBeFalsy();
    });
  });
  describe('the owner', () => {
