...
describe("certificateSummaryBuilder", () => {
  ...
  describe('events', () => {
    it("should work with only arianeeEvents", () => {
      ...
      expect(arianeeCertificate.events.all).toEqual([{ blockNumber: 0 }, { blockNumber: 2 }]);
    ...
    it("should work with only events", () => {
      ...
      expect(arianeeCertificate.events.all).toEqual([{ blockNumber: 0 }, { blockNumber: 1 }]);
    ...
    it("should have all events sorted", () => {
      ...
      expect(arianeeCertificate.events.all).toEqual([{ blockNumber: 0 }, { blockNumber: 1 }, { blockNumber: 2 }]);
